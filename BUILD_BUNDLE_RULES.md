...
Скрипт сравнивает по sha256 три копии — вывод сборки, bundle в репозитории и установленный
bundle — и называет, какое звено разошлось. Ни сборка, ни установщик, ни handshake хоста об
этом не сообщают: каждый из них рапортует успех, пока Navisworks держит старую DLL. Если
установки нет вовсе, это не дрейф, и скрипт говорит это отдельно. Аргументом можно передать
другой корень, например системный остаток в `ProgramData`, которого быть не должно.
...
никогда не собирались, обязан отказаться подтверждать установку, а не молчать.
