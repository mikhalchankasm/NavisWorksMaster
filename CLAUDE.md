...
`AIColorService.cs` calls the DeepSeek API (configurable endpoint). Configuration is loaded by `AIConfig.cs` from environment variables or `%APPDATA%\NavisHelper\ai_config.json`. The nested `LocalColorBridge` and `LocalColorService` implementations are in `AIColorObjects.cs`; the bridge provides file-based IPC between the Navisworks process and the standalone `ColorService/` executable, with fallback to direct API calls.
