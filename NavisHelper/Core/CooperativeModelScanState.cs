...
namespace NavisHelper.Core
{
    ...
    internal sealed class CooperativeModelScanChunkPolicy
    {
        ...
        /// <summary>
        /// Scale of the within-root progress asymptote, in items. The total
        /// item count is unknown before the traversal finishes, so within-root
        /// progress uses processed/(processed + scale). The scale must stay
        /// large enough that the bar keeps visibly moving on multi-million
        /// item models instead of parking near completion while the dominant
        /// root is still being scanned (the 94/94 plateau defect).
        /// </summary>
        internal const double RootProgressAsymptoteScale = 65536.0;
            ...
            int currentRootProcessed)
        {
            ...
            // Keep the fraction strictly below 1.0 inside a sub-operation:
            // the scan phase must not visually complete before the apply
            // phase has run.
            var rootFraction = currentRootProcessed <= 0
                ? 0.0
                : Math.Min(
                    0.99,
                    currentRootProcessed /
                    (currentRootProcessed + RootProgressAsymptoteScale));
            return Math.Max(
                0.0,
                Math.Min(0.9999, (completedRoots + rootFraction) / totalRoots));
        }
    }
