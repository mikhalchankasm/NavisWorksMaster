...
- **The native search addresses properties by internal name, and a model may have none.** Measured live on `6501.5.nwd`, Navisworks 2027, same build and same condition: `scope=whole_model` with `matchDepth=all` and `DN equals "150mm"` returned **0 matches** while `scope=under_named_node` over a 22-node subtree returned **1**, with `depthHistogram {5:1}` and `sampleValuesFromModel ["150mm"]`. So the engine cannot see a property the manual traversal matches, and `WholeModelCountOnlyWarning` asserts that `matchedItemCount` is *exact* while it happens. `item_properties_by_handle` on a node of that branch shows why: every `internalName` is an empty string and the categories and properties carry display names only (`Элемент` / `Имя`, `AVEVA` / `DN`), while a native `SearchCondition` is built from internal names and `MatchesSearchManually` reads the display side. This is **not** the pruning asymmetry above and is not fixed by scoping: it is why `whole_model` `find_items`, the default routing of the tool everything else is built on, can silently under-report on a federated RVM model. Not yet diagnosed to a fix; do not route more requests onto the native path on the argument that pruning makes its result exact — that argument is about which matches are returned, not about which properties can be seen. Draft #28 was stopped for exactly that reason.
