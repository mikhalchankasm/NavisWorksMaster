...
- **A `ModelItem` set costs what it keeps alive, not what it hashes:** a dedup set fed every match was the only reference to 41 016 wrappers in `clash_create_matrix_from_selection`, and bounding it by the items the call returns took the call from 7–10 s to 2.2 s (#78); the same for `find_items_by_bbox` (#79) and the scoped traversal's `visited` (#80) gave 2× and 5–8×. Removing lookups whose items another collection keeps alive anyway (`find_items`' sort, `model_color_scheme`'s caches) changed nothing measurable. When a walk needs identity dedup, bound the set by what it returns unless something else already holds those wrappers.
