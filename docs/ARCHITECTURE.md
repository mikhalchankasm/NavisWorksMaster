...
- **Completed walks also leave native wrappers behind:** `HeavyWorkCollectionPolicy` triggers after four generation-0 collections since the last forced collection. After a gated request finishes, the host runs a full collection and finalizer drain off the UI thread and outside the request gate so later calls do not inherit the wrapper backlog.
