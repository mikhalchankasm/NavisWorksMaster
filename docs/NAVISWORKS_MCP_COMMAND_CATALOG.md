...
| `export_selection_geometry` | `implemented` | Выгружает исходные треугольники в OBJ/JSONL/PLY/STL с точными путями узлов, фрагментами и матрицами. Читает текущую выборку или `scope=match_handle`; `apply=false` считает без записи, `apply=true` атомарно создаёт файл. Без сглаживания и децимации. Параметры и ограничения: [инструкция](MCP_GEOMETRY_EXPORT.md) | `выгрузи сетку оборудования`, `получи треугольники штуцера`, `экспортируй геометрию по хэндлу` |
| `selection_export_properties` | `implemented` | CSV/XLSX свойств текущего выделения либо напрямую по `scope=match_handle`, без потери узлов с одинаковым путём. `cleanValues=true` даёт типизированные значения с десятичной точкой; `ItemIndex` различает узлы. По умолчанию dry-run | `выгрузи атрибуты найденных узлов`, `сохрани свойства без изменения выделения` |
| `list_item_children` | `implemented` | Возвращает непосредственных детей по `parentMatchHandle` или точному отображаемому `parentPath`, включая имена со слешами. Неоднозначный путь требует хэндла. `offset`/`nextOffset` дают страницы после фильтра скрытых узлов; каждый ребёнок имеет `matchHandle`, вся страница — `childrenMatchHandle`. `parentName`/`sourceFile` используют индекс корней | `покажи непосредственных подчиненных`, `дай следующую страницу детей`, `получи хэндл этого узла` |
...
| `export_selection_geometry` | `ExportSelectionGeometry` | `NavisHelper.McpServer/Tools/NavisworksGeometryTools.cs` | Exports native tessellated triangles with exact node paths, fragment identities and transforms. No smoothing or decimation. Read-only model access; apply=false counts and validates without writing. Limits fail rather than publishing partial meshes. |
