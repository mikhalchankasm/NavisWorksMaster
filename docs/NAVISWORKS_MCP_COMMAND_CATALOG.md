...
| `last_operation_status` | `LastOperationStatus` | `NavisHelper.McpServer/Tools/NavisworksTools.cs` | Returns host-side status for a recent request_id, or for the most recent host operation when no request_id is given. Use after request_timeout, transport disconnect, or oversized-response suspicion to determine whether the Navisworks-side command eventually completed, failed, or is still running. |
