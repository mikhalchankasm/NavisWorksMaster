...
| `world_markers_manage` | `implemented` | Прячет, показывает, удаляет и очищает сохранённые world-метки по `names`, `ids` или `group`. Пустой (blank) селектор отказывается, а запрос вообще без селектора действует на все метки: `clear` без селектора удаляет всё. Метки — тот же overlay: не геометрия модели, не сохраняются с документом и сбрасываются при его смене. По умолчанию dry-run | `спрячь метки зоны А`, `покажи все метки`, `удали метку насос`, `очисти все world-метки` |
