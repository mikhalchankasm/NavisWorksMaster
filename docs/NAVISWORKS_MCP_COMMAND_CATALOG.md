...
| `clash_bbox_pair_plan` | `implemented` | Планировщик пар групп перед Clash Detective: берёт верхнеуровневые model/root items как `list_root_items`, пропускает ancestor/descendant self-пары, проверяет пересечение bbox, опционально уточняет на 1-2 уровня внутрь дерева (`refineDepth=1/2`) с внутренними лимитами, возвращает candidate pairs, skipped reasons и warnings. Не меняет модель, не создаёт и не запускает clash tests; `outputPath` записывается только при `apply=true`, preview остаётся без файловых изменений. Использовать первым шагом, чтобы не строить матрицу все-со-всеми для сотен групп | `найди пересекающиеся группы по bbox`, `построй план clash pairs`, `сократи матрицу коллизий по bounding box`, `проверь какие файлы пересекаются` |
| `clash_pair_tests_create` | `implemented` | Создаёт Clash Detective tests из пар `clash_bbox_pair_plan` или JSON `planOutputPath`: имена вида `NH-BBOX 0001 - A vs B`, selection A/B по найденным root items, `toleranceMm`, `testType`, лимит первых N. По умолчанию dry-run; `apply=true` только создаёт tests и не запускает их. `overwriteExisting=false` защищает от перезаписи существующих tests | `создай clash tests из bbox пар`, `создай первые 5 проверок NH-BBOX`, `сделай проверки группа против группы`, `подготовь тесты коллизий по плану` |
...
| `clash_pair_tests_create` | `ClashPairTestsCreate` | `NavisHelper.McpServer/Tools/NavisworksClashTools.cs` | Creates Clash Detective tests from bbox candidate pairs. Defaults to dry-run; pass apply=true only after reviewing planned test names/selections. This does not run created tests. |
