    ...
    public static McpErrorContractResponse GetErrorContract()
    {
        return new McpErrorContractResponse
        {
            Errors = new List<McpErrorContractItem>
            {
                ...
                Error(ErrorCodes.SectionBoxNotEnabled, "The active view has no enabled oriented Section Box.", "Enable and position a Section Box manually, then call get_current_section_box again.", false),
                Error(ErrorCodes.SectionBoxModeUnsupported, "Sectioning is enabled in plane mode rather than oriented-box mode.", "Switch Navisworks Sectioning Tools to Box mode, then retry capture.", false),
                Error(ErrorCodes.SectionBoxPayloadUnsupported, "The clipping payload is malformed or uses an unsupported format/version.", "Do not replay guessed geometry. Recreate the Section Box in a supported Navisworks version and retry capture.", false),
                Error(ErrorCodes.SectionBoxUnitsMismatch, "The captured box units differ from the active document units.", "Do not rescale or guess. Replay the scenario only in its strict matching document context, or capture a new box for this document.", false),
